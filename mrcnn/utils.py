...
def unmold_mask(mask, bbox, image_shape):
    ...
    """
    # Keep a copy of the input mask
    original = mask

    threshold = 0.5

    # Resize mask to the size of bounding box
    y1, x1, y2, x2 = bbox
    mask = resize(mask, (y2 - y1, x2 - x1))
    original = resize(original, (y2 - y1, x2 - x1))
    ...
    scored_mask = np.zeros(image_shape[:2], dtype=np.float32)

    full_mask[y1:y2, x1:x2] = mask
    scored_mask[y1:y2, x1:x2] = original

    return full_mask, scored_mask 
